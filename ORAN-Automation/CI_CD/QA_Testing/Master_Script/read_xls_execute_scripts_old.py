...
def create_ini_file(config_param):
    ...
    CUPlane = ['test_mac_ip', 'test_mac_username', 'test_mac_password', 'vxt_add', 'du_mac', 'external_gain']
    ...
    Telnet = ['port_id','telnet_user','telnet_password']
...
def read_xls_for_fetching_config_and_test_cases(ru_name, version):
    ru_keyword = {"LPRUB_4T4R":"lpru","LPRUC_4T4R":"lpru","LPRUD_4T4R":"lpru","BND28_4T4R":"bn28","BND1_2T2R":"mcb1","VVDN_8T8R":"do_latr"}
    df = pd.ExcelFile('{}/test_case.xlsx'.format(dir_name))
    df1 = pd.read_excel(df, header=0, nrows=47)
    df2 = pd.read_excel(df, header=0, skiprows=48)
    config_param_dict = {}
...
def Create_required_directories(ru_name, version):
    try:
        ...
        time_format = time.strftime("%d_%m_%M_%I_%M_%p", present_time)
...
def execute_tc_xls(df, ru_name, Version, Pass, Fail, Skip):
    ...
    for i, row in df.iterrows():
        ...
        new_filename = f'{root_dir}/Master_Script/xml_files/temp_{i}.xml'
        if i > 0:
        ...
        if execute == 'yes':
            ...
            if str(arguments) == 'nan':
                process = subprocess.Popen(
                    ['{0}/CI_CD/bin/python'.format(root_dir), '{0}/{1}'.format(absolute_script_path, script_name)])
            else:
                # print(arguments,type(arguments),str(arguments))
                process = subprocess.Popen(['{0}/CI_CD/bin/python'.format(
                    root_dir), '{0}/{1}'.format(absolute_script_path, script_name), str(arguments)])
            timeout_flag = False
            ...
            if return_code == 0:
                append_pass_in_xml_file(
                    script_name, plane, "pass", elapsed_time)
                executed_test_case.append([script_name, 'Pass'])
                Pass += 1
            elif return_code == 100 and ru_name == "LPRU":
                append_fail_in_xml_file(
                    script_name, plane, "fail", elapsed_time, return_code)
    ...
    new_filename = f'{root_dir}/Master_Script/xml_files/temp_{i+1}.xml'
    with open(new_filename,'w+'):
        print(f'{new_filename} is created.')
    shutil.copyfile(f"{dir_name}/temp.xml",new_filename)
