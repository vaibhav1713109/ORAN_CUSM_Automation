...
/**
 *
 * @file     sw_update_negative_case.py
...
configur.read('{}/require/inputs.ini'.format(parent_dir))
...
class o_ru_sw_update_negative_case_3_1_6_2(PDF,Configuration,netopeer_connection,genrate_pdf_report):
    ...
    def netopeer_connection_and_capability(self,cmd,ethtool_out,ping_output):
        try:
            ...
            if self.session:
                ...
                Result = netopeer_connection.add_test_description(self,Test_Description=Test_Desc, Test_Case_name='M_CTC_ID_015')
                if len(Result) > 2:
    ...
    def software_download(self):
        try:
            ...
            ###############################################################################
            xml_data = open("{}/require/Yang_xml/sw_download.xml".format(parent_dir)).read()
            ...
            while time.time() < t:
                ...
                if 'download-event' in notify:
                    xml_pretty_str = x.toprettyxml()
                    # print(xml_pretty_str)
                    if 'COMPLETED' not in notify:
    ...
    def software_install(self):
        try:
            ...
            ###############################################################################
            xml_data2 = open("{}/require/Yang_xml/sw_install.xml".format(parent_dir)).read()
            file_path = self.rmt
            ...
            while time.time() < t:
                ...
                x = xml.dom.minidom.parseString(notify)
                # print(xml_pretty_str)
                if 'install-event' in notify:
                    xml_pretty_str = x.toprettyxml()
                    # print(xml_pretty_str)
                    if 'INTEGRITY_ERROR' not in notify:
                        PDF.STORE_DATA(self,'{}'.format(xml_pretty_str),Format='XML')
            ...
            if not xml_pretty_str:
                Configuration.append_data_and_print(self,f'Notification not captured || Fail !!')
                return f'Notification not captured || Fail !!'
            Configuration.append_data_and_print(self,f'Software install || {self.flash_image} Successful!!')
            ###############################################################################
            ## POST_GET_FILTER
            ###############################################################################            
            PDF.add_page(self)
            ...
            PDF.STORE_DATA(self,xml_pretty_sw_inventory, Format='XML', )
            Configuration.append_data_and_print(self,f'Software {self.flash_image} Install || Not install on {self.inactive_slot} due to corrupt image!!')
            return True
    ...
    def check_image(self):           
        list_image = os.popen('ls -lrt {}'.format(configur.get('INFO','corrupt_image_path')))
    ...
    def Main_Function(self):
        ...
        try:
            ...
            self.sw_file = '/'.join((configur.get('INFO','corrupt_image_path'),self.flash_image))
    ...
    def api_call(self):
        ...
        Test_procedure = [f"{'='*100}\nTest case *SW Update Negative case* Started!! Status: Running\n{'='*100}",'** Test Coverage **'.center(50),'Static IP Ping',
                'Netopeer Connection','Capability exchange and create-subscription','Configure SW Download',
                'Configure SW Install','Installation failed due to corrupt image', 'Start Time : {}'.format(start_time),'='*100]
        notification('\n'.join(Test_procedure))
        Result = self.Main_Function()
        Configuration.Result_Declartion(self,'SW Update',Result, 'M_CTC_ID_015')
...
if __name__ == "__main__":
    obj = o_ru_sw_update_negative_case_3_1_6_2()
    Result = obj.api_call()
