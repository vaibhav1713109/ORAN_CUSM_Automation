    ...
    def netopeer_connection_and_capability(self,cmd,ethtool_out,ping_output):
        try:
            ...
            if self.session:
                ...
                Result = netopeer_connection.add_test_description(self,Test_Description=Test_Desc, Test_Case_name='SW Update')
                if len(Result) > 2:
            ...
            while time.time() < t:
                ...
                if 'download-event' in notify:
                    ...
                    if 'COMPLETED' not in notify:
                        Configuration.append_data_and_print(self,f'Software File Download || {self.flash_image} Fail!!')
    ...
    def software_activate(self):
        try:
            ...
            if not xml_pretty_str:
                Configuration.append_data_and_print(self,f'Notification not captured || Fail !!')
                return f'Notification not captured || Fail !!'
            Configuration.append_data_and_print(self,f'Software activate || {self.flash_image} Successful!!')
            ###############################################################################
            ## POST_GET_FILTER
            ###############################################################################
            time.sleep(5)
