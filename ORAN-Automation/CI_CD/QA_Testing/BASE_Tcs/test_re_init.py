...
###***************************************************************************#
###$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  Get_RU_specific  $$$$$$$$$$$$$$$$$$$$$$$$$#
IP_DHCP_interface = configur.get('INFO','static_dynamic')
...
Telnet_username = configur.get('Telnet', 'Username')
Telnet_password = configur.get('Telnet', 'Password')
###******************************************************************************#
###$$$$$$$$$$$$$$$$$$$$$$$$$$  Get_PowerSupply_config $$$$$$$$$$$$$$$$$$$$$$$#
is_RPS_Or_Kiku = configur.get('INFO','RPS_Or_Kiku')
if "RPS" in is_RPS_Or_Kiku:
    ...
    Kiku_visa_addrs = configur.get('INFO','Kiku_visa_addrs')
###******************************************************************************#
############################################### Geting Params #########################################
...
class initialisation():
    ...
    def find_high_speed_interface(self,input1):
...
def Dynamic_Connection():
    config = configparser.ConfigParser()
    config.read('/home/sebu.mathew/inputs.ini')

    host = config.get('Dynamic', 'ru_ip')
    port = config.getint('Dynamic', 'port')
    username = config.get('Dynamic', 'username')
    password = config.get('Dynamic', 'password')

    try:
        device = manager.connect(host=host, port=port, username=username, password=password, hostkey_verify=False)
        print("Connection Established")
        return device
...
def get_interface_informations_from_RU(connection, filtr):
    try:
        get_res=get_filter_command(connection, filtr)
        if(get_res):
            ...
            print(result_mac)
            result_ip=read_xml_for_a_leaf(xml_object,'interface', 'ip')
            print(result_ip)
            append_to_ini_file('{}/inputs.ini'.format(root_dir),'RUNNING',{'interface':result_interf[1],'ru_ip':result_ip[1]})
        else:
            return 100
    except Exception as e:
        print(f"Failed to execute 'get' command: {e}")
...
if __name__ == "__main__":
    if "Not_initialised" in init_status:
        ...
        if any(item in "LPRUB_4T4R" for item in TelnetRus) and static_dynamic== 'Static': #No need of Restart use Static_IP to fetch
            ...
            init = initialisation()
            is_ping = init.check_ping_status(Telnet_ip)
            if is_ping:
                print("connecting to the netopeer")
                NPC = NP_Connection(Telnet_ip)
                if NPC:
                    print("Netopeer Connection established successfully")
                    Result = get_interface_informations_from_RU(NPC, var1)
                    sys.exit(0)
                else:
            ...
            append_to_ini_file('{}/inputs.ini'.format(root_dir),'Dynamic',{'ru_ip',ip_list[1]})
            FREF_Offs,deltaF_Global,NREF_Offs = identify_Fref_deltF_Nref(tx_freq)
            tx_Nref = str(int(NREF_Offs + (tx_freq-FREF_Offs) / deltaF_Global))
            print(int(tx_Nref))
            FREF_Offs,deltaF_Global,NREF_Offs = identify_Fref_deltF_Nref(rx_freq)
            rx_Nref = str(int(NREF_Offs + (rx_freq-FREF_Offs) / deltaF_Global))
            print(int(rx_Nref))
            #using dynamic_IP connect
            possible_ru_IP = configur.get('Dynamic','ru_ip')
            if possible_ru_IP:
                init = initialisation()
                is_ping = init.check_ping_status(possible_ru_IP)
                if is_ping:
                    print("connecting to the netopeer")
                    NPC = Dynamic_Connection()
                    if NPC:
                        print("Netopeer Connection established successfully")
                        Result = get_interface_informations_from_RU(NPC, var1)
                        sys.exit(0)
                    ...
                    print("Ping is not obtained --> with Possible IP....restarting RU to fetch Possible IP")
            append_to_ini_file('{}/inputs.ini'.format(root_dir),'INFO',{'tx_arfcn':tx_Nref,'rx_arfcn':rx_Nref},'RUNNING')
        ...
        if ip_list == None:
            ...
            if not init.check_ping_status(ip_list[0]):
                print("Ping not obtained...in Dynamic IP, Please Check connection")
                sys.exit(202)
        append_to_ini_file('{}/inputs.ini'.format(root_dir),'RUNNING',{'ru_ip':result_ip[1]}) #fh_interface
        append_to_ini_file('{}/inputs.ini'.format(root_dir),'RUNNING',{'init_state','initialised'})
        sys.exit(0)    
    else:
        possible_ru_IP = configur.get('RUNNING','ru_ip')
        if possible_ru_IP:
            init = initialisation()
            is_ping = init.check_ping_status(possible_ru_IP)
            if is_ping:
                ...
                if not init.check_ping_status(ip_list[0]):
                    print("Ping not obtained...in Dynamic IP, Please Check connection")
                    sys.exit(302)
                append_to_ini_file('{}/inputs.ini'.format(root_dir),'RUNNING',{'ru_ip':result_ip[1]})
                sys.exit(0)
