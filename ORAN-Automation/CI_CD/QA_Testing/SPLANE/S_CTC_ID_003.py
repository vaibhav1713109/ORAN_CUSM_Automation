    ...
    try:
        ...
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            if state == "Link" or "Event":
                ...
                pdf.ln(5)
                notification("Waiting of O-RU synchronization")
                list1.append("Waiting of O-RU synchronization")
                ###########################################################################################################################
                ...
                while timeout > time.time():
                    n = m.take_notification(timeout=300)
                    if n == None:
                        break
                ...
                ###########################################################################################################################
                STARTUP.STORE_DATA('\t\t\t Retriving the nominal Clock class and ssm code',Format = True,PDF = pdf)  
                SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
                <sync xmlns="urn:o-ran:sync:1.0">
                </sync>
                </filter>
                ...
                xml_pretty_str = x.toprettyxml()
                STARTUP.STORE_DATA(xml_pretty_str,Format = "XML",PDF = pdf)
                notification("O-RU Synchronized || Success")
                list1.append("O-RU Synchronized || Success")
                ###########################################################################################################################
                #                                        Configure clock class 7 from paragon                                                           #
                ###########################################################################################################################
                pdf.add_page()
                STARTUP.STORE_DATA("\t\t\t configure clock class 7 from Master",Format = True,PDF = pdf)
                calnexSet(f"app/mse/master/Master{port_num}/clockclass", "ClockClass", 7)
                calnexSet("app/mse/applypending")
                time.sleep(15)
                SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
                <sync xmlns="urn:o-ran:sync:1.0">
                ...
                xml_pretty_str = x.toprettyxml()
                STARTUP.STORE_DATA(xml_pretty_str,Format = "XML",PDF = pdf)
                notification("Changing the clock class to 7 and 135 (degraded inside range) || Success")
                list1.append("Changing the clock class to 7 and 135 (degraded inside range) || Success")
                ###########################################################################################################################
                #                                        Configure clock class 135 from paragon                                                           #
                ###########################################################################################################################
                pdf.add_page()
                STARTUP.STORE_DATA("\t\t\t configure clock class 135 from Master",Format = True,PDF = pdf)
                calnexSet(f"app/mse/master/Master{port_num}/clockclass", "ClockClass", 135)
                calnexSet("app/mse/applypending")
                time.sleep(15)
                STARTUP.STORE_DATA('Expected Result',Format = True,PDF = pdf)
                ###########################################################################################################################
                #                                             Retrieving Final state of O-RU                                                          #
                ###########################################################################################################################
                SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
                <sync xmlns="urn:o-ran:sync:1.0">
                </sync>
                ...
                xml_pretty_str = x.toprettyxml()
                STARTUP.STORE_DATA(xml_pretty_str,Format = "XML",PDF = pdf)
                notification("Retrieving the final configuration of O-RU || Success")
                list1.append("Retrieving the final configuration of O-RU || Success")
