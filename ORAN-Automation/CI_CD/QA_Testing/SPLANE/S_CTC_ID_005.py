import socket, sys ,os ,json ,os ,xmltodict ,xml.dom.minidom ,time ,paramiko ,STARTUP,subprocess,re
    ...
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            try:
                ...
                STARTUP.STORE_DATA("Can't find the value_SYNC",Format = False,PDF = pdf)
            ...
            STARTUP.STORE_DATA(f"{'SyncE_State STATUS' : <50}{'=' : ^20}{'PASS' : ^20}" if SyncE_state == "NOK" else f"{'SyncE_state STATUS' : <50}{'=' : ^20}{'FAIL' : ^20}",Format = False,PDF = pdf)
    except Exception as e:
        STARTUP.STORE_DATA(e,Format = False,PDF = pdf)
        exc_type, exc_obj, exc_tb = sys.exc_info()
    ...
    try:
        ...
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            if state == "Link" or "Event":
                ...
                notification("O-RU Synchronized status || Waiting")
                list1.append("O-RU Synchronized status || Waiting")
                time.sleep(6)
                pdf.add_page()
                ###########################################################################################################################
                ...
                while timeout > time.time():
                    n = m.take_notification(timeout=180)
                    if n == None:
                        break
                ...
                xml_pretty_str = x.toprettyxml()
                STARTUP.STORE_DATA(xml_pretty_str,Format = "XML",PDF = pdf)
                notification("O-RU Synchronized status || Success")
                list1.append("O-RU Synchronized status || Success")
                pdf.add_page()
                ###########################################################################################################################
                #                                        Configure clock class 140 and DNU from paragon                                                           #
                ###########################################################################################################################
                STARTUP.STORE_DATA("Configure clock class 140  & Change SSM_CODE to DNU from master ",Format = True,PDF = pdf)
                calnexSet(f"app/generation/synce/esmc/Port{port_num}/ssm", "SsmValue", 'QL-DNU')
                calnexSet(f"app/mse/master/Master{port_num}/clockclass", "ClockClass", 140)
                calnexSet("app/mse/applypending")                            
                time.sleep(3)
                ...
                pdf.add_page()
                ###########################################################################################################################
                #                                        Configure clock class 150 from paragon                                                           #
                ###########################################################################################################################
                STARTUP.STORE_DATA("configure clock class 150 from Master",Format = True,PDF = pdf)
                calnexSet(f"app/mse/master/Master{port_num}/clockclass", "ClockClass", 150)
                calnexSet("app/mse/applypending")                            
                time.sleep(10)
                SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
                <sync xmlns="urn:o-ran:sync:1.0">
                ...
                pdf.add_page()
                ###########################################################################################################################
                #                                        Configure clock class 160 from paragon                                                           #
                ###########################################################################################################################
                STARTUP.STORE_DATA("configure clock class 160 from Master",Format = True,PDF = pdf)
                calnexSet(f"app/mse/master/Master{port_num}/clockclass", "ClockClass", 160)
                calnexSet("app/mse/applypending")                            
                time.sleep(10)
                ...
                pdf.add_page()
                ###########################################################################################################################
                #                                        Change the Domain number from paragon                                                           #
                ###########################################################################################################################
                STARTUP.STORE_DATA('changing the Domain number to 25',Format = True,PDF = pdf)
                calnexSet(f"app/mse/master/Master{port_num}/domain", "Domain", 25)
                calnexSet("app/mse/applypending")
                
                time.sleep(5)
                notification("Domain number changed || Success")
                list1.append("Domain number changed || Success")
                c = 0  
                timeout = time.time() + 200
                while timeout > time.time():
                    n = m.take_notification(timeout=180)
                    ...
                    for key, value in recursive_items(output_dict):
                        NF1 = key
                    c += 1
                    # if NF1=='sync-state' and c == 3:
                    if c == 2 or c == 3:
                        break
                pdf.add_page()
                ###########################################################################################################################
                #                                            Retrieving Holdover state                                                           #
                ###########################################################################################################################
                ...
                xml_pretty_str = x.toprettyxml()
                STARTUP.STORE_DATA(xml_pretty_str,Format = "XML",PDF = pdf)
                notification("O-RU Transit to freerun || Waiting")
                list1.append("O-RU Transit to freerun || Waiting")
                global dict_Sync1
                dict_Sync1 = xmltodict.parse(str(data1))
                time.sleep(900)
                ###########################################################################################################################
                #                                             Retrieving Freerun state                                                           #
                ###########################################################################################################################
                ...
                xml_pretty_str = x.toprettyxml()
                STARTUP.STORE_DATA(xml_pretty_str,Format = "XML",PDF = pdf)
                notification("O-RU Transit to freerun || Success")
                list1.append("O-RU Transit to freerun || Success")
            else:
