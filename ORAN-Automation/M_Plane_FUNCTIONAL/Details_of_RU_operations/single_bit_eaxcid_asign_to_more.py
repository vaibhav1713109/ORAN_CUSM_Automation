###############################################################################
##@ FILE NAME:      Same bit to BAND-SECTOR-ID, CC-ID, DU-PORT-ID and for RU-PORT-ID
    ...
    def test_procedure(self):
        try:
            ...
            Test_Step1 = """STEP 1 & 2 The NETCONF Client assigns unique eAxC_IDs to low-level-rx-endpoints. The same set of eAxC_IDs is also assigned to low-level-tx-endpoints. The NETCONF Client uses <rpc><editconfig>.
                        Configure same bits to the BAND-SECTOR-ID, CC-ID, DU-PORT-ID and for RU-PORT-ID. """
...
def test_m_ctc_id_026():
    ...
    if Check == False:
        ...
        STARTUP.ACT_RES(f"{'Same bit to BAND-SECTOR-ID, CC-ID, DU-PORT-ID and for RU-PORT-ID' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=[255,0,0])
        return False
    ...
    Exp_Result = 'Expected Result : 1. Netoconf client revert back with <RPC-REPLY> with error tag.'
    STARTUP.STORE_DATA(Exp_Result, Format='DESC', PDF=pdf)

    STARTUP.STORE_DATA('\t\t{}'.format('****************** Actual Result ******************'), Format=True, PDF=pdf)
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'Same bit to BAND-SECTOR-ID, CC-ID, DU-PORT-ID and for RU-PORT-ID' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=[0,255,0])
            return True
