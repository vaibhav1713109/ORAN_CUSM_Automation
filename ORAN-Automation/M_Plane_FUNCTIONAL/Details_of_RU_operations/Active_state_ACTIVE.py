###############################################################################
##@ FILE NAME:      communication between related U-Plane endpoints is enabled 
#                   when tx-array-carrier or rx-array-carrier value of parameter 
#                   “active” is “ACTIVE” and value of parameter “state” is “READY”.
#                   Otherwise communication is disabled.
...
class M_CTC_ID_026(vlan_Creation):
    ...
    def test_procedure(self):
        try:
            ...
            ###############################################################################   
            pdf.add_page()          
            Test_Step1 = """STEP 1 & 2 The NETCONF Client assigns unique eAxC_IDs to low-level-rx-endpoints. The same set of eAxC_IDs is also assigned to low-level-tx-endpoints. The NETCONF Client uses <rpc><editconfig>.
                        Assign  tx-array-carrier or rx-array-carrier value of parameter “active” is “ACTIVE” and value of parameter “state” is “READY"""
            ...
            xml_1 = open('{}/require/Yang_xml/active_leaf.xml'.format(parent)).read()
            ...
            ###############################################################################   
            pdf.add_page()            
            Test_Step2 = "STEP 3 The O-RU NETCONF Sever responds with the <rpc-reply> message indicating compeletion of the requested procedure."
            ...
            ###############################################################################   
            pdf.add_page()            
            Test_Step4 = "STEP 4 Using <get --filter-xpath /o-ran-uplane-conf:user-plane-configuration> validate the configured value. "
            ...
            active_RX1 = USER_PLANE['data']['user-plane-configuration']['rx-array-carriers']['active']
            active_TX1 = USER_PLANE['data']['user-plane-configuration']['tx-array-carriers']['active']
            state_TX1 = USER_PLANE['data']['user-plane-configuration']['tx-array-carriers']['state']
            state_RX1 = USER_PLANE['data']['user-plane-configuration']['rx-array-carriers']['active']

            # self.session.close_session()
            ################# Check the ARFCN #################
            if (ARFCN_RX1 == self.rx_arfcn) and (ARFCN_TX1 == self.tx_arfcn) and (active_RX1 == active_TX1 == self.active_state):
                if state_RX1 == state_TX1 == 'READY':
                    STARTUP.STORE_DATA(xml_pretty_str,Format='XML', PDF=pdf)
                    return True
                else:
                    return "state of RU is {}".format(state_RX1)    
            else:
    ...
    def test_Main_026(self):
        ...
        self.active_state = input('Enter Active state of RU : ')
            ...
            if self.session:
                ...
                for key, val in self.RU_Details[1].items():
                    if val[0] == 'true' and val[1] == 'true':
                        ...
                        ## Test Description
                        ###############################################################################
                        Test_Desc = '''Test Description : 1. O-RU is powered ON and has an ethernet connection to the DHCP-server
                            2. M-Plane NETCONF session established.
                            '''
                        CONFIDENTIAL = STARTUP.ADD_CONFIDENTIAL('099', SW_R=val[2])
...
def test_m_ctc_id_026():
    ...
    if Check == False:
        ...
        STARTUP.ACT_RES(f"{'Active_state_ACTIVE' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=[255,0,0])
        return False
    ...
    Exp_Result = 'Expected Result : Communication between related U-Plane endpoints is enabled under condition "Assign  tx-array-carrier or rx-array-carrier value of parameter “active” is “ACTIVE” and value of parameter “state” is “READY”."'
    STARTUP.STORE_DATA(Exp_Result, Format='DESC', PDF=pdf)

    STARTUP.STORE_DATA('\t\t{}'.format('****************** Actual Result ******************'), Format=True, PDF=pdf)
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'Active_state_ACTIVE' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=[0,255,0])
            return True
