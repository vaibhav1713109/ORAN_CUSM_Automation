###############################################################################
##@ FILE NAME:      NETCONF Client can put the tx-array-carrier / rx-array-carrier to 
#                   sleep by setting value of parameter “active” in the corresponding 
#                   tx-array-carrier element / rx-array-carrier element to “SLEEP”.
    ...
    def test_procedure(self):
        try:
            ...
            Test_Step1 = """STEP 1 & 2 The NETCONF Client assigns unique eAxC_IDs to low-level-rx-endpoints. The same set of eAxC_IDs is also assigned to low-level-tx-endpoints. The NETCONF Client uses <rpc><editconfig>.
                        Set value of parameter “active” in the corresponding tx-array-carrier element / rx-array-carrier element to “SLEEP” and the value of its parameter “state” is “READY"""
...
def test_m_ctc_id_026():
    ...
    if Check == False:
        ...
        STARTUP.ACT_RES(f"{'Active_State_SLEEP' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=[255,0,0])
        return False
    ...
    Exp_Result = 'Expected Result : When value of its parameter “active” is “SLEEP” and value of its parameter “state” is “READY”.'
    STARTUP.STORE_DATA(Exp_Result, Format='DESC', PDF=pdf)

    STARTUP.STORE_DATA('\t\t{}'.format('****************** Actual Result ******************'), Format=True, PDF=pdf)
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'Active_State_SLEEP' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=[0,255,0])
            return True
