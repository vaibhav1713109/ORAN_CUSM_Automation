...
class hw_usage_state(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Test step for genrating the hw_usage_state
            ###############################################################################                
            xml_data = open("xml/hw_usage_state.xml").read()
            u1 =f'''
                    <config>
                    {xml_data}
                    </config>'''
        ...
        try:
            ...
            for key, val in self.slots.items():
                if val[0] == 'true' and val[1] == 'true':
                    ############################### Test Description #############################
                    Test_Desc = '''Test Description : 1. Verify whether Netconf Client is able to change usege-state defined in o-ran-hardware of the O-RU by using remote procedure call reset.'''
...
2. Verify O-RU "usege-state" starts from disabled when O-RU receives reset.
3. Verify O-RU Controller is able to reset the O-RU, even if the O-RU state is "disabled" or "enabled”.
            ...
            if result != True:
                ...
                STARTUP.ACT_RES(f"{'hw_usage_state' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'hw_usage_state' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python hw_usage_state.py <Test_Case_ID>')
