...
class hw_availability_state(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Test step for genrating the hw_availability_state
            ###############################################################################                
            xml_data = open("xml/hw_availability_state.xml").read()
            u1 =f'''
                    <config>
                    {xml_data}
                    </config>'''
        ...
        try:
            ...
            for key, val in self.slots.items():
                if val[0] == 'true' and val[1] == 'true':
                    ############################### Test Description #############################
                    Test_Desc = '''Test Description : 1. Check the O-RU "availability-state" using <get rpc>
2. O-RU controller will initiate <normal>when the equipment is functioning correctly 
3. O-RU controller will initiate < degraded >when the equipment is reporting a major/critical alarm
4. O-RU controller will initiate < faulty > Ability of the equipment to continue its operation when it is impacted by the critical alarms 
'''
            ...
            Exp_Result = '''Expected Result : 1. Validate in ORU availability state indicates "normal" which have no fault.
2. Validate in ORU availability state indicates "degraded" which have critical fault.
3. Validate in ORU availability state indicates "faulty" which indicates O-RU can't continue any services
                '''
            ...
            if result != True:
                ...
                STARTUP.ACT_RES(f"{'hw_availability_state' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'hw_availability_state' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python hw_availability_state.py <Test_Case_ID>')
