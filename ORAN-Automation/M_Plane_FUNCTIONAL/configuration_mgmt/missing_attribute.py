###############################################################################
##@ FILE NAME:      Error-tag : missing-attribute
...
class missing_attribute(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Test step for genrating the missing_attribute
            ###############################################################################                
            xml_data = open("xml/missing_attribute.xml").read()
            u1 =f'''
                    <config>
                    {xml_data}
                    </config>'''
        ...
        try:
            ...
            for key, val in self.slots.items():
                if val[0] == 'true' and val[1] == 'true':
                    ############################### Test Description #############################
                    Test_Desc = '''Test Description : Test to verify whether from the NETCONF Error List with tag missing-attribute'''
...
Description:    An expected element is missing.
            ...
            if result != True:
                ...
                STARTUP.ACT_RES(f"{'Error_Tag[missing-attribute]' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'Error_Tag[missing-attribute]' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python Missing_attribute.py <Test_Case_ID>')
