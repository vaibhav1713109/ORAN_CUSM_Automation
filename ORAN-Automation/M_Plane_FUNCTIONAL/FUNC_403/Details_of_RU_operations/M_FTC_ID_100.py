###############################################################################
##@ FILE NAME:      NETCONF Client performs deactivation by setting the value of 
#                   the parameter “active” at tx-array-carrier element /rx-array-carrier element to “INACTIVE”
    ...
    def test_procedure(self):
        try:
            ...
            Test_Step1 = """STEP 1 & 2 The NETCONF Client assigns unique eAxC_IDs to low-level-rx-endpoints. The same set of eAxC_IDs is also assigned to low-level-tx-endpoints. The NETCONF Client uses <rpc><editconfig>.
                        Assign  tx-array-carrier or rx-array-carrier value of parameter “active” is “INACTIVE” """
            ...
            if (ARFCN_RX1 == self.rx_arfcn) and (ARFCN_TX1 == self.tx_arfcn) and (active_RX1 == active_TX1 == self.active_state):
                if state_RX1 == state_TX1 == 'DISABLE':
                    STARTUP.STORE_DATA(xml_pretty_str,Format='XML', PDF=pdf)
                    return True
                else:
            ...
            if self.session:
                ...
                for key, val in self.RU_Details[1].items():
                    ...
                    2. M-Plane NETCONF session established.
                        '''
                        CONFIDENTIAL = STARTUP.ADD_CONFIDENTIAL('100', SW_R=val[2])
...
def test_m_ctc_id_026():
    ...
    if Check == False:
        ...
        STARTUP.ACT_RES(f"{'Active_state_INACTIVE' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=[255,0,0])
        return False
    ...
    Exp_Result = 'Expected Result : Communication is disabled under condition "Assign  tx-array-carrier or rx-array-carrier value of parameter “active” is “INACTIVE” "'
    STARTUP.STORE_DATA(Exp_Result, Format='DESC', PDF=pdf)

    STARTUP.STORE_DATA('\t\t{}'.format('****************** Actual Result ******************'), Format=True, PDF=pdf)
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'Active_state_INACTIVE' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=[0,255,0])
            return True
