###############################################################################
##@ FILE NAME:      Rejected file Management Operation download
...
class reject_file_retrieve(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Test step 2 Configure file retrieve RPC
            ###############################################################################
            xml_data = open("Yang_FUNC_xml/M_FTC_ID_081.xml").read()


            STARTUP.STORE_DATA('******** TER NETCONF Client triggers <file-retrieve> with "INVALID"  path ************',Format='TEST_STEP',PDF=pdf_log)
        ...
        try:
            ...
            for key, val in self.slots.items():
                if val[0] == 'true' and val[1] == 'true':
                    ############################### Test Description #############################
                    Test_Desc = '''Test Description : Test to validate the failed reason for the file retrieval process.'''
...
2. Check O-RU output status respond with rejected in the request 
3. O-RU send the notification for rejection with error message/code. 
'''
            ...
            if result:
                ...
                STARTUP.ACT_RES(f"{'Rejected file Management Operation download' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'Rejected file Management Operation download' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python Reject_file_retrieve.py <Test_Case_ID>')
