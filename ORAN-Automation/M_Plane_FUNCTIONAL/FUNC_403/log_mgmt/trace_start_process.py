###############################################################################
##@ FILE NAME:      Check the trace start process
...
class trace_start_process(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Test step 1 configure trace start RPC
            ###############################################################################                
            """STARTUP.STORE_DATA('********** Configure trace start RPC ***********',Format=True,PDF=pdf_log)
            xml_data = open("xml/M_FTC_ID_056.xml").read()
            rpc_reply = m.dispatch(to_ele(xml_data))
            ###############################################################################
            ## Test step 2 Wait for the rpc reply message.
            ###############################################################################
            STARTUP.STORE_DATA(rpc_reply, Format="XML",PDF=pdf_log)"""
            
            ###############################################################################
            ## Test step 2 Wait for 15 minutes for logs to get generated on O-RU.
            ###############################################################################
            time.sleep(900)

            ###############################################################################
            ## Test step 4 Send the <retrieve-file-list> for logs to get generated with below inputs:
            ###############################################################################                
            # xml_data = open("xml/M_FTC_ID_056.xml").read()
            # u1 =f'''
            ...
            #         {xml_data}
            #         </config>'''
            # rpc_reply_2 = m.dispatch(to_ele(xml_data))
            # STARTUP.STORE_DATA(rpc_reply_2, Format="XML",PDF=pdf_log)
            # dict_data = xmltodict(str(d))

            ###############################################################################
            ## Test step 4 Send the <file-upload> for logs to get generated logs with below inputs:
            ###############################################################################                
            # xml_data = open("xml/M_FTC_ID_056.xml").read()
            # rpc_reply_3 = m.dispatch(to_ele(xml_data))
            # STARTUP.STORE_DATA(rpc_reply_3, Format="XML",PDF=pdf_log)
            return True
        ...
        try:
            ...
            for key, val in self.slots.items():
                if val[0] == 'true' and val[1] == 'true':
                    ############################### Test Description #############################
                    Test_Desc = '''Test Description : Test to validate the success file retrieval process.'''
            ...
            Exp_Result = '''Expected Result : 1. Verify NETCONF client opened successfully.
2. Check O-RU output status respond with success in the request 
3. O-RU send the notification for success.. 
'''
            ...
            if result:
                ...
                STARTUP.ACT_RES(f"{'Check the trace start process' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'Check the trace start process' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python trace_start_process.py <Test_Case_ID>')
