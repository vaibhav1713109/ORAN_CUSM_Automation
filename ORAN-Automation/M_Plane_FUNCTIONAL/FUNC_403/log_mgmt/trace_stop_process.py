###############################################################################
##@ FILE NAME:      Check the trace stop process
...
class trace_stop_process(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Test step 1 configure trace stop RPC
            ###############################################################################                
...
'''
            ...
            if result:
                ...
                STARTUP.ACT_RES(f"{'Check the trace stop process' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'Check the trace stop process' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python trace_stop_process.py <Test_Case_ID>')
