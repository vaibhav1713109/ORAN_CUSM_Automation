###############################################################################
##@ FILE NAME:      Success file Management Operation Download
...
class success_file_download(vlan_Creation):
        ...
        try:
            ...
            for key, val in self.slots.items():
                if val[0] == 'true' and val[1] == 'true':
                    ############################### Test Description #############################
                    Test_Desc = '''Test Description : Test to validate the success file download process.'''
...
'''
            ...
            if result:
                ...
                STARTUP.ACT_RES(f"{'Success file Management Operation Download' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'Success file Management Operation Download' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python success_file_download.py <Test_Case_ID>')
