...
def main(test_case_name,eaxid,amplitude,ext_gain):
    ...
    for _ in range(2):
        try:
            if 'PRACH' not in test_case_name:
                ...
                if scp_gen_status != True and retry_flag == True:
                    retry_flag = True
            ...
            if 'PRACH' not in test_case_name:
                ...
                if pcap_gen_status != True and retry_flag == True:
                    retry_flag = True
            ...
            if pcap_load_and_Play_gen_status != True and retry_flag == True:
                retry_flag = True
            ...
            if 'UL' in test_case_name or 'PRACH' in test_case_name:
                if 'PRACH' not in test_case_name:
                    ...
                    if orb_gen_status != True and retry_flag == True:
                        retry_flag = True
        ...
        print("Reetuen flag = ",retry_flag)
        if 'DL' in test_case_name and 'UL' in test_case_name:
            # print(DL_data[eaxcid],UL_data[eaxcid])
            if 'Pass' in DL_data[eaxid] and 'Pass' in UL_data[eaxid]:
                retry_flag = True
            if retry_flag == True:
...
if __name__ == "__main__":
    ...
    eaxids = configur.get('INFO','eaxcids').split(',')
    amplitudes = configur.get('INFO','amplitude').split(',')
    external_gain = configur.get('INFO','external_gain').split(',')
    RU_Name = configur.get('INFO','ru_name')
    img_version = configur.get('INFO','img_version')
    ru_serial_no = configur.get('INFO','ru_serial_no')
    rf_switch_port = configur.get('INFO','rf_switch_port').split(',')
    testing_type = configur.get('INFO','testing_type')
    bandwidth = configur.get('INFO','bandwidth')
    ...
    dir_status = make_directories(eaxids)
    if dir_status != True:
        print(dir_status)
        sys.exit(1000)
    ...
    csv_file_data = []
    csv_file_path = f"{root_dir}/Results/{testing_type}/{RU_Name}/{img_version}/{ru_serial_no}/{bandwidth}/Test_Report_ANT{'_'.join(eaxids)}_{time_format}.csv"
    ...
    for switch_port,eaxcid,amplitude,ext_gain in zip_list:
        ...
        if chn_status!=True:
            sys.exit(10000)
        report_path = f"{root_dir}/Results/{testing_type}/{RU_Name}/{img_version}/{ru_serial_no}/{bandwidth}/EAXCID{eaxcid}/Eaxcid_{eaxcid}_{time_format}.pdf"
        DL_UL_Results = {}
        for test_case in test_cases:
            ...
            if 'Comp_9' in test_case and flag_comp_9 == False:
                ...
                if change_comp_status != True:
                    ...
                    sys.exit(1000)
                notification("\n******************** Compression Change to 16********************\n")  
            Result = main(test_case,eaxcid,amplitude,ext_gain)
            # print(Result)
            if 'Error' in Result:
            ...
            if 'DL' in test_case and 'UL' in test_case:
                ...
                ul_data,ul_crc = list(Result[0].values())
            ...
            json_file = open(f"{root_dir}/Results/{testing_type}/{RU_Name}/{img_version}/{ru_serial_no}/{bandwidth}/EAXCID{eaxcid}/Eaxcid_{eaxcid}_{time_format}.json", "w+")
            json.dump(DL_UL_Results, json_file,indent=4)
            json_file.close()
            read_file = open(f"{root_dir}/Results/{testing_type}/{RU_Name}/{img_version}/{ru_serial_no}/{bandwidth}/EAXCID{eaxcid}/Eaxcid_{eaxcid}_{time_format}.json", "r")
            json_data = json.load(read_file)
            # print(DL_UL_Results)
            generate_report_dl_ul(json_data,report_path)
            read_file.close()
            csv_file_1.close()
