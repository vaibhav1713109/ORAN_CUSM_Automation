...
from Oran_GUI_Pyqt5.Priya_GUI import Ui_CU_TDD_HomePage
...
class MainWindow_tdd(QtWidgets.QMainWindow):
        ...
        for key, val in self.ui_cu.gethered_info.items():
            if type(val) == QtWidgets.QLineEdit:
                if 'Power_limit' in key.text() and len(val.text().strip())!=0 and val.text().isdigit():
                    input_data['_'.join(key.text().split())] = '{0} {1}'.format(int(val.text())-1.5,int(val.text())+1.5)
                    print(key.text(), '{0} {1}'.format(int(val.text())-1.5,int(val.text())+1.5))
                elif 'Externaldelaytime' in key.text() and len(val.text().strip())!=0:
                    if 'ns' in val.text():
                        input_data['_'.join(key.text().split())] = format((float(val.text()[:-2])*(10**(-9))),'.9f')
                        print(key.text(), int(val.text()[:-2])*(10**(-9)))
                    elif 'ms' in val.text():
                        input_data['_'.join(key.text().split())] = format((float(val.text()[:-2])*(10**(-3))),'.3f')
                        print(key.text(), int(val.text()[:-2])*(10**(-3)))
                elif len(val.text().strip())!=0:
                    input_data['_'.join(key.text().split())] = val.text()
                ...
                input_data['_'.join(key.text().split())] = val.currentText()
    ...
    def run_module(self,console):
        ...
        # content = self.comboBox.currentText()
            ...
            if self.p == None:
                ...
                self.result = self.p.start("python", ["{}/TDD/TDD_Config.py".format(dir_path)])
    ...
    def openSaveDialog(self):
        ...
        # QFileDialog.setStyleSheet(self,'background-color:black')
        # l = 'hello world'
        ...
        # QFileDialog.setStyleSheet(self,'background-color:white')
