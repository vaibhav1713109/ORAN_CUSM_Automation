...
def session_login(host, port, user, password):
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            driver.get("http://172.17.80.4/#/testMeasurement/playlist")
            ...
            if element == "detected":
                ...
                while True:
                        ...
                        if NF1=='ptp-state':
                            ...
                            break
                pdf.add_page()
                STARTUP.STORE_DATA("Retrieving SYNCE Locked State",Format = True,PDF = pdf)
                ...
                STARTUP.STORE_DATA("Changing QL_SSM to DNU",Format = True,PDF = pdf)
                STARTUP.SYNCE_DNU()
                pdf.image(name=f'{d1}'+'/SYNCE_DNU.png', x = None, y = None, w = 180, h = 70, type = '', link = '')
                pdf.ln(5)
