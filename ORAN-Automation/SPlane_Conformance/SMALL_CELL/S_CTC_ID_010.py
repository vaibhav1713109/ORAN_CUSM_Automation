...
def session_login(host, port, user, password):
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            calnexSet("app/mse/master/Master2/stop")
...
if __name__ == '__main__':
    ...
    try:
        ...
        STARTUP.STORE_DATA(e,Format = False,PDF = pdf)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        STARTUP.STORE_DATA('Error Line Number:{exc_tb.tb_lineno}',ormat = False,PDF = pdf)


    calnexSet("app/mse/master/Master2/clockclass", "ClockClass", 6)
    calnexSet("app/mse/applypending")
    calnexSet("app/generation/synce/esmc/Port2/ssm", "SsmValue", 'QL-PRC')
    calnexSet("app/mse/master/Master2/stop")
    calnexSet("app/generation/synce/esmc/Port2/stop")