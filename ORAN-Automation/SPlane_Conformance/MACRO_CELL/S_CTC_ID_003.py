    ...
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            if element == "detected":
                ...
                # calnexSet("app/mse/master/Master2/clockclass", "ClockClass", 7)
                # calnexSet("app/mse/applypending")
                STARTUP.clock_class_7()
                pdf.image(name=f'{d1}'+'/clock_class_7.png', x = None, y = None, w = 180, h = 70, type = '', link = '')
                pdf.ln(5)
                time.sleep(15)
                SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
                <sync xmlns="urn:o-ran:sync:1.0">
                ...
                # calnexSet("app/mse/master/Master2/clockclass", "ClockClass", 135)
                # calnexSet("app/mse/applypending")
                STARTUP.clock_class_135()
                pdf.image(name=f'{d1}'+'/clock_class_135.png', x = None, y = None, w = 180, h = 70, type = '', link = '')
                pdf.ln(5)
                time.sleep(15)
...
if __name__ == '__main__':
    try:
        ...
        # places = []
        # with open(f'{d1}/myfile1.txt', 'r') as filehandle:
        ...
        # sid = m.session_id
        # kill_ssn("192.168.1.10",830,usr,pss,sid)
        
        
        # res = session_login("192.168.1.10",830,usr,pss)
