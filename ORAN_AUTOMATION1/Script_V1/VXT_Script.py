...
def run_scpi_cmnd(InstrObj, scpi_cmds, sub_folder):
    ...
    print("sleep start")
    time.sleep(5)
    InstrObj.write(":MMEM:STOR:SCR 'capture.png'")
    print("print taken")
    status = InstrObj.query('*OPC?')
    while(1):
    ...
    ResultData = bytes(InstrObj.query_binary_values(f'MMEM:DATA? "{image}"', datatype='s'))
...
def Constellation_check_DL_TDD_Comp_1(bands, sub_folder, DL_Center_Freq, Power_Value, VXT_Add, Ext_Gain):
    ...
    time.sleep(2)
    print (InstrObj)
    scpi_cmds = [':RAD:STAN:PRES:RBAL DLTM3DOT1A', ":EVM:CCAR0:PDSC1:MCS:TABL TABL2", ":EVM:CCAR0:PDSC1:MCS 27", ":EVM:CCAR0:PDSC2:MCS:TABL TABL2", ":EVM:CCAR0:PDSC2:MCS 27", ":EVM:CCAR0:PDSC3:MCS:TABL TABL2", ":EVM:CCAR0:PDSC3:MCS 27", ":EVM:CCAR0:PDSC4:MCS:TABL TABL2", ":EVM:CCAR0:PDSC4:MCS 27"]
    a = ":CORR:BTS:GAIN "+str(Ext_Gain)
    scpi_cmds.append(a)
    scpi_cmds.append(":MMEM:STOR:SCR 'lat.png'")
    run_scpi_cmnd(InstrObj, scpi_cmds, sub_folder)
